...
class Writer():
    ...
    def add_element(self, annotation, annotation_value, region, depends,
        ...
        if new_file:
            document = self.graf.create_xml_graph_header(annotation, depends)
        ...
        if annotation_value is not '':
            if region is None and depends is not '':
                document = self.graf.create_node_annotation_no_id(document, annotation,
                ...
                document = self.graf.create_node_with_region_no_id(document, depends,
...
class Parser():
    """
    This class contain the methods to the
    parser the GrAF files and write to
    Annotation Tree.
